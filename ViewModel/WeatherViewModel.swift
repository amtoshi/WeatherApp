...
//  WeatherApp
//
//  Created by Ashutosh Mane on 24/07/22.
//
...
public class WeatherViewModel {
  ...
  //MARK: DATA Binders initialization
  ...
  //MARK: this feeds the fetchWeatherForLocation, so if the location isnt found, then the error is handled here BUT weather API is handled in the fetchWeatherForLocation fuction
  ...
  private func fetchWeatherForLocation(_ location: Location) {
    WeatherbitService.weatherDataForLocation(
      ...
      longitude: location.longitude) { [weak self] (weatherData, error) in
        ...
        //MARK: this triggers the data binders didSet()
